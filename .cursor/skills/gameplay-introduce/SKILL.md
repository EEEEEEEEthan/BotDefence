...
| 方法 | 说明 |
|------|------|
| `move(direction: int)` | 向四向移动一步，direction 为 Consts.Direction.NORTH/SOUTH/EAST/WEST |
...
func run(bot) -> void:
	bot.move(Consts.Direction.NORTH)
	bot.move(Consts.Direction.EAST)
	bot.move(Consts.Direction.SOUTH)
	bot.move(Consts.Direction.WEST)
```

## 架构要点

- **Bot**：玩家可见封装，仅暴露 `move`；子线程调用时 call_deferred 交给 BotMain，抵达后 semaphore 解除阻塞
