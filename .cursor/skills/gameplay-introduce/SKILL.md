...
| `move(direction: int)` | 向四向移动一步，direction 为 Consts.Cardinal.NORTH/SOUTH/EAST/WEST，抵达后更新 cardinal |
...
- **Bot**：玩家可见封装，暴露 `cardinal`、`move`；子线程调用时 call_deferred 交给 BotMain，抵达后 semaphore 解除阻塞
